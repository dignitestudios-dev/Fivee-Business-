...
export function DynamicAssetRows({
  title,
  assets,
  onAddAsset,
  onRemoveAsset,
  onUpdateAsset,
}: DynamicAssetRowsProps) {
  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  onUpdateAsset(asset.id, "description", e.target.value)
                }
                className="focus:ring-[#22b573] focus:border-[#22b573]"
              />
            </div>
            <div>
              <Label htmlFor={`${asset.id}-marketValue`}>
                Market Value ($)
              </Label>
              <Input
                ...
                onChange={(e) =>
                  onUpdateAsset(asset.id, "marketValue", e.target.value)
                }
                className="focus:ring-[#22b573] focus:border-[#22b573]"
              />
            </div>
            <div>
              <Label htmlFor={`${asset.id}-loanBalance`}>
                Loan Balance ($)
              </Label>
              <Input
                ...
                onChange={(e) =>
                  onUpdateAsset(asset.id, "loanBalance", e.target.value)
                }
                className="focus:ring-[#22b573] focus:border-[#22b573]"
              />
            </div>
